...
## עדכון — סיבוב פידבק אחרי פריסה לפרודקשן

v2.1 **פורס בפועל** (schema+seed רצו ב-Query editor, ה-API פורס ידנית עם
`func azure functionapp publish` — ה-GitHub Actions workflow **לא עובד**, ה-OIDC login
נכשל תמיד וללא secrets מוגדרים; הפריסה היחידה שעובדת היא הידנית). לאחר שהמשתמש התחיל
לעבוד בפועל בפורטל, הגיע סיבוב פידבק גדול אחד עם 9 סעיפים — כולם טופלו:

1. כפתור "צפייה כמשתמש" (view-as, read-only) למנהלי IT/SuperAdmin.
2. תיקון 400 גורף בכל ה-`create`/`update` — היה deployment ישן שלא תואם לסכימה החדשה;
   נפתר ע"י redeploy.
3. טופס "הוסף משתמש": שם פרטי/משפחה באנגלית בראש הטופס וממלא אוטומטית שם
   משתמש+מייל (אותיות קטנות+נקודה+2 מהשם משפחה), ניתן לעריכה ידנית; שדה מייל
   בפורמט `username` + סיומת `@דומיין` קבועה בצד; סיסמה ניתנת לעריכה + כפתור העתקה
   משולב (מייל+סיסמה יחד).
3ב. אשף 3 שלבים לסקירת בקשת הקמת משתמש (`userRequestDetailBackdrop`) עם progress
   bar ירוק: פרטי עובד → סקריפט הקמה (סיסמה ניתנת לעריכה) → סיום+שליחת מייל אוטומטי
   למגיש הבקשה עם פרטי ההתחברות.
4. בקשות הקמת משתמש מוזגו לתוך טבלת/דשבורד הקריאות (כרטיס סגול, "קח משימה").
5. כפתור "תור קריאות" הוסר; הדשבורד ממוין לפי דחיפות (חמור→קל), כרטיסים מהבהבים
   בצבע הדחיפות הגבוהה ביותר.
6. כפתור "סגור קריאה" מוסתר אלא אם הקריאה כבר נלקחה לטיפול (`בטיפול`).
7. כפתורי סינון בדשבורד (משתמש+סניף) הוחלפו בכפתורים מעוגלים עם popover בחירה מרובה.
8. **מערכת קטגוריות/תת-קטגוריות/דרגות-דחיפות דינמית**, ניתנת לניהול מלא ע"י IT Admin
   (כפתור חדש "סוגי קריאות" ב-hub): טבלאות `TicketCategories`/`TicketSubcategories`
   (עם `IsDynamic`+`DynamicSource`)/`TicketUrgencyLevels` (+`Tickets.Subcategory`),
   entity חדש `api/src/entities/ticketConfig.js`, מסך אדמין מלא (CRUD לכל השלושה).
   תת-קטגוריה "דינמית" לדוגמה: "מדפסת / סורק" → "בחר מדפסת" עם
   `DynamicSource='printers-by-branch'` — מציג בטופס הקריאה את רשימת המדפסות בסניף
   המשתמש (הכל אם "מרוחק"), ע"י שימוש חוזר ב-`populatePrinterSelect`. **החלטת scope
   מכוונת**: מנגנון תת-הקטגוריה הדינמית נשאר **נפרד** מה-toggle הקיים "עבור:
   מחשב/מדפסת" שכבר קיים בטופס (שמנתב מייל ל-`PRINTER_SUPPORT_EMAIL`) — כדי לא
   לערבב בין שני מנגנונים שכבר עובדים; `Subcategory` נשמר לצורך IT בלבד, לא מניע
   ניתוב מייל. דחיפויות/צבעים הוגדרו כ-data (`ColorHex`+`Severity`) והרינדור
   (`buildTicketCard`, הפילים בטופס) עבר מ-CSS classes קבועים ל-inline styles
   מבוססי-נתונים, כדי שהוספת/שינוי דרגה ע"י IT Admin תשתקף מיד בלי שינוי קוד.
9. עיצוב מחדש כפתורי הדחיפות בטופס קריאה — קומפקטיים יותר, "גבוהה" + הסבר קצר לצד.

**מה נשאר אחרי הסיבוב הזה**: להריץ שוב את `infra/schema.sql`+`infra/seed.sql`
(idempotent, בטוח לרוץ מהתחלה) ואז לפרוס מחדש את ה-API
(`func azure functionapp publish it-portal-api-490fb4 --javascript`) — התהליך הרגיל.
לאחר מכן לבדוק שהקטגוריות/דרגות מוצגות נכון בטופס קריאה ושמסך "סוגי קריאות" (IT
Admin) עובד. הייבוא החד-פעמי של המשתמשים מ-365 עדיין לא בוצע (בכוונה — נשאר אחרון,
ראה `infra/README.md`).

## עדכון — לולאת התחברות אינסופית ב-`portal.ramilevystock.com` (אחרי המעבר ל-Azure Static Web Apps)

**התסמין**: אחרי המעבר מ-GitHub Pages ל-Azure Static Web App (SWA) + דומיין חדש, ניסיון
התחברות נכנס ללולאה אינסופית: מסך התחברות של Microsoft → אישור אפליקציה/הרשאות
(אוטומטי) → מסך הטעינה של הפורטל ("טוען...") → וחזרה למסך ההתחברות. שוב ושוב, בלי
להיכנס בפועל.

**מה גילינו, שכבה אחר שכבה** (כל שכבה טופלה בנפרד, אבל לא פתרה את כל הבעיה):

1. **`msal-browser.min.js` נטען מ-CDN חיצוני** (`cdn.jsdelivr.net`) — Tracking Prevention
   של Edge חסם לספרייה גישה ל-`sessionStorage` (כי זה סקריפט מדומיין שלישי), ומנע מ-MSAL
   לשמור מצב בין קפיצות ההתחברות. **תוקן**: אחסון מקומי (`js/vendor/msal-browser.min.js`)
   — גם פתר חלק מהבעיה וגם הסיר תלות חיצונית לספריית אבטחה קריטית (side benefit קבוע).
2. **Cache בדפדפן** — אחרי התיקון הקודם זה עבד בחלון אנונימי אך לא בחלון רגיל, כי
   הדפדפן "נעל" גרסה שבורה ישנה במטמון. **תוקן**: `Cache-Control: no-cache` על
   `index.html` ב-`staticwebapp.config.json`, כך שכל דיפלוי עתידי נכנס לתוקף מיד לכולם.
3. **הסיבה האמיתית — שתי מערכות AAD על אותו דומיין**: ניתוח Network tab מלא (ללא
   סינון) חשף ששער ההתחברות המובנה של SWA עצמו (`routes: [{route:"/*",
   allowedRoles:["authenticated"]}]`, שנועד לחסום גישה אנונימית לקבצים הסטטיים) **רץ
   בו-זמנית** עם ההתחברות הנפרדת של MSAL מול אפליקציית ה-SPA. ההתנגשות גרמה ל-Microsoft
   להפעיל widget פנימי למיזוג/בחירת-חשבון (`Me.htm` מ-`aadcdn.msauth.net`), ושער ה-SWA
   הפעיל ניסיון התחברות מחדש שוב ושוב — לולאה אמיתית, לא תקלת קוד רגילה.
4. **מה ניסינו ולא עבד**: פיצול `index.html` (שער בלבד, ללא MSAL) מ-`app.html` (האפליקציה
   בפועל), בתקווה שהפרדת-הזמנים תמנע את ההתנגשות. **לא פתר** — כי MSAL חייב לחזור
   (redirect URI) בדיוק לעמוד שהאפליקציה נמצאת בו, וברגע שגם אותו עמוד מוגן ע"י שער
   ה-SWA, ההתנגשות חוזרת באותה הנקודה בדיוק (מאומת ב-Network tab פעם שנייה).

**מסקנה**: שער ההתחברות המובנה של Azure Static Web Apps **לא תואם ארכיטקטונית** לאפליקציה
עם MSAL משלה מול Entra ID — בלי קשר ל-tier (Free/Standard) או למבנה העמודים.

**הפתרון הסופי שיושם**: הסרת שער ה-SWA **לגמרי**. `staticwebapp.config.json` כיום מכיל רק
`Cache-Control: no-cache` על `index.html` ו-`/`, ושני headers אבטחה (`X-Frame-Options`,
`X-Content-Type-Options`) — בלי `allowedRoles`/`responseOverrides`. משמעות: קבצי ה-frontend
(HTML/CSS/JS) נגישים ללא אימות, בדיוק כמו קוד המקור הציבורי (View Source) של כל אתר —
ואינם מכילים שום סוד (טופל כבר קודם בתיקון חשיפת ה-GitHub Pages המלאה). ההגנה האמיתית על
הנתונים היא **אך ורק ברמת ה-API**:
- Easy Auth על ה-Function App — טוקן AAD תקף חייב להיות מונפק ספציפית לטננט הזה (חשבונות
  מארגונים/אישיים אחרים לא יכולים להתחבר בכלל).
- חסימת "unprovisioned users" ב-`dispatch.js` — גם עובד עם טוקן תקף ייחסם ב-403 אם IT לא
  רשם אותו בטבלת המשתמשים.
- בהמשך: Conditional Access (MFA + מכשיר מנוהל) ייאכף ברמת אפליקציות ה-API/SPA עצמן (לא
  ברמת ה-SWA) — זה השלב הבא (P6) בתוכנית.

**שיפורים שנשארו בקוד (side benefits מהדרך, שווה לשמר)**:
- `msal-browser.min.js` מאוחסן מקומית תחת `js/vendor/`, לא מ-CDN חיצוני.
- הגנה מובנית נגד לולאת redirect ב-`loadIdentity()` (`js/api-client.js`): ניסיון
  interactive אחד בלבד לכל טעינת עמוד (flag ב-`sessionStorage`) — אם עדיין אין חשבון
  אחרי ניסיון קודם, נזרקת שגיאה ומוצג כפתור "נסה שוב" (`js/main.js`) במקום לופ שקט.

**לקח לעתיד, אם ירצו שוב לחסום גישה אנונימית לקבצים הסטטיים** (מעבר להגנת ה-API בלבד):
- **Cloudflare Access** מול ה-SWA — מערכת זהות נפרדת לגמרי מ-AAD (לא נוגעת ב-
  `login.microsoftonline.com`), ולכן לא מתנגשת עם MSAL. הפתרון הפשוט ביותר אם ירצו לחזור
  לזה.
- **ארכיטקטורת זהות אחת**: לוותר על MSAL בפרונט, להסתמך רק על שער ה-SWA + "linked
  backend" שמעביר את הזהות המאומתת ל-API (`x-ms-client-principal` header). נקי יותר
  ארכיטקטונית אך דורש שינוי משמעותי באופן שה-API מזהה קריאות.
- **לא לנסות שוב לשלב שער AAD מובנה של SWA + MSAL עצמאי על אותו דומיין** — לא עובד, מוכח
  פעמיים בפועל (גם לפני וגם אחרי פיצול העמודים).
